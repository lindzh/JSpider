...
public class SpiderContextImpl implements SpiderContext {
	...
	@Override
	public void preHandle(Map<String, String> header, Site site) {
//        connection.setDefaultUseCaches(false);
//        connection.setUseCaches(false);
        header.put("Cache-Control","max-age=0,no-cache");
        header.put("Pragma","no-cache");
        if (useProxyAuthentication) {
        	header.put("Proxy-Authorization", authenticationString);
        }
	...
	@Override
	public void postHandle(HttpResponseMeta response, Site site) {
		List<Header> headers = response.getHeaders();
        setCookies(site, cookieUtil.getCookies(headers));
